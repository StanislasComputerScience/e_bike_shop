...
def create_database_with_json_file() -> dict:
    """Create your database"""
    try:
        folder = "bdd/"
        with open(cv.JSON_BDD_PATH) as db_json_file:
            ecommerce_db_dict = json.load(db_json_file)

        connection = sqlite3.connect(cv.BDD_PATH)
        cursor = connection.cursor()
        for key, sql_command in ecommerce_db_dict.items():
            print(key)
            cursor.execute(sql_command)
        ...
        print(f"{cv.BDD_PATH} already exist !")
...
def main():
    ...
    create_database_with_json_file()
